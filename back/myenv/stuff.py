...
def insertAbsence(request, teacher_id):
    ...
    if serializer.is_valid():
        serializer.save(IdProf_id = teacher_id)
...
def insertSeance(request,teacher_id):
    ...
    if serializer.is_valid():
        print(request.data)
        serializer.save(Matricule_id = teacher_id)
        return Response(serializer.data, status=201)
    else :
        return Response(serializer.errors, status=400)
...
def creer_absence(request,teacher_id):
...
def modifier_enseignant(request, matricule):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response("Les informations de l'enseignant ont été mises à jour avec succès.")
    return Response(serializer.errors, status=400)
